...
: IRequestHandler<DeleteCourseBookingCommand>
    {
        public async Task<Unit> Handle(DeleteCourseBookingCommand request, CancellationToken ct)
        {
            var data = await _uow.CourseBookingRepo.GetByIdAsync(request.Id, ct: ct)
